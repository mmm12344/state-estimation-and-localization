...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    q_hat = Quaternion(axis_angle=e_s[6:9]).quat_mult_left(Quaternion(*q_check))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    f = imu_f.data[k-1]
    w = imu_w.data[k-1]
    p_est[k] = p_est[k-1] + delta_t * v_est[k-1] + (delta_t**2/2) * (C_ns @ f + g)
    v_est[k] = v_est[k-1] + delta_t * (C_ns @ f + g)
    q_est[k] = Quaternion(axis_angle=w * delta_t).quat_mult_right(q_est[k-1])
    # 1.1 Linearize the motion model and compute Jacobians
    F_k = np.eye(9)
    F_k[0:3, 3:6] =  delta_t * np.eye(3)
    F_k[3:6, 6:9] = -(skew_symmetric(C_ns @ f)) * delta_t
    
    Q_k = np.zeros((6,6))
    Q_k[0:3, 0:3] = delta_t**2 * np.eye(3) * var_imu_f
    Q_k[3:6, 3:6] = delta_t**2 * np.eye(3) * var_imu_w
    L = l_jac
    H = h_jac
    # 2. Propagate uncertainty
    p_cov[k] = F_k @ p_cov[k-1] @ F_k.T + L @ Q_k @ L.T 
